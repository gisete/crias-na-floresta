...
export default function TestimonialSlider({ testimonials }: TestimonialSliderProps) {
  ...
  return (
    <div className="flex items-center gap-10 relative z-20">
      ...
      {/* Quote Text */}
      <div className="flex-1 min-w-0 md:h-[420px] flex flex-col justify-center">
        <div
          className={`transition-opacity duration-300 ${isAnimating ? 'opacity-0' : 'opacity-100'}`}
        >
          <p className="text-sm md:text-[15px] leading-[1.9] font-light opacity-90 max-w-[620px]">
            "{currentTestimonial.quote}"
          </p>
          {currentTestimonial.author && (
            <p className="text-xs md:text-[13px] mt-4 opacity-70 italic">— {currentTestimonial.author}</p>
          )}
        </div>
      </div>
